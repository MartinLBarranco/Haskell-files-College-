...
digitos :: Integer -> [Integer]
digitos n | length(show n) == 1 = [n]
          | otherwise = read[head(show n)] ++ digitos k
              where k = read(init(show n))                     --ERROR EXTRAÑO
-- ----------------------------------------------------------------------------
...
multiplicacionRusa :: Integer -> Integer -> Integer
multiplicacionRusa a b = undefined
...
listaMenoresIguales :: Ord a => a -> [a] -> [a]
listaMenoresIguales x ys = undefined
